...
app.post('/api/telegram-webhook', async (req, res) => {
  ...
  try {
    ...
    if (text === '/news') {
      ...
      try {
        ...
        const marketButtons = sorted.map(m => [{
          ...
          url: `https://crediplex.name.ng?market=${m.id}`
        }]);
    ...
    if (text === '/help') {
      await sendTelegramMessage(chatId,
        ...
        `/bet — Place a bet directly from Telegram\n` +
        `/cashout — Cash out an active bet early\n` +
        `/stats — Your full trading statistics\n` +
        `/withdraw — Withdraw your funds\n` +
        `/copytrade — Copy a top trader automatically\n` +
        ...
        { reply_markup: { inline_keyboard: [
          ...
          [{ text: '🎯 Bet', callback_data: 'bet' }, { text: '💸 Cashout', callback_data: 'cashout' }],
          [{ text: '📊 Stats', callback_data: 'stats' }, { text: '💸 Withdraw', callback_data: 'withdraw' }],
          [{ text: '🔄 Copy Traders', callback_data: 'copytrade' }],
          [{ text: '🎧 Support', callback_data: 'support' }, { text: '🔐 PIN Help', callback_data: 'support' }],
          [{ text: '🚀 Open Crediplex', url: 'https://crediplex.name.ng' }]
        ]}}
      );
      return;
  ...
  // ── Deposit freetext flow ──
    ...
    if (betSt) {
      if (betSt.step === 'awaiting_keyword') {
  ...
  // ── Support AI flow — fires if in support state OR if no other state matches ──
    const supportSt = _supportState[chatId.toString()];
    const noOtherState = !_alertState[chatId.toString()] && !_depositState[chatId.toString()] && !_withdrawState[chatId.toString()];
    if (msg.text && !msg.text.startsWith('/') && (supportSt || noOtherState)) {
      // Auto-init support state if not exists
      if (!_supportState[chatId.toString()]) {
        _supportState[chatId.toString()] = { messages: [] };
      }
