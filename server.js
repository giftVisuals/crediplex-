...
// Stagger startup: wait 3 minutes before first sync, then every 2 hours
setTimeout(runSyncThenRefreshCache, 3 * 60 * 1000);
setInterval(runSyncThenRefreshCache, 2 * 60 * 60 * 1000);
console.log('🔄 Auto Polymarket sync started (every 2 hours, first run after 3min)');
  ...
  try {
    ...
    if (text === '/help') {
      await sendTelegramMessage(chatId,
        ...
        `/unsubscribe — Stop notifications & unlink account\n` +
        `/help — Show this menu`
      );
      return;
    }
...
const MARKETS_CACHE_TTL = 30 * 60 * 1000; // 30 minutes
...
setTimeout(refreshMarketsCache, 8000);
...
setTimeout(refreshTopTradersCache, 12000);
