...
function detectCategoryAdmin(question, tags) {
  ...
  const combined = q + ' ' + t;
  if (t.includes('world-cup') || t.includes('world cup') || t.includes('fifa-world-cup') || combined.includes('world cup') || combined.includes('fifa world cup')) return 'World Cup';
...
// Firestore trigger — watch for new notifications and push them
async function watchNewNotifications(){
  let lastCheck = Date.now();
  setInterval(async ()=>{
    try {
      const since = admin.firestore.Timestamp.fromDate(new Date(lastCheck - 5000));
      lastCheck = Date.now();
      ...
      for(const d of snap.docs){
        const n = d.data();
        const title = n.title||'Crediplex';
        const body = n.message||'You have a new notification';
        if(n.targetAll){
          const usersSnap = await db.collection('users').where('fcmToken','!=','').limit(200).get();
          await Promise.all(usersSnap.docs.map(u=>sendFcmToToken(u.data().fcmToken,title,body)));
        } else if(n.uid){
          const userSnap = await db.collection('users').doc(n.uid).get();
          const token = userSnap.data()?.fcmToken;
          if(token) await sendFcmToToken(token,title,body);
        }
      }
    } catch(e){}
  }, 5 * 60 * 1000); // check every 5 min — now a backup net only, since admin broadcasts push instantly via /api/send-push
}
