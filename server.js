    ...
    try {
      ...
      for(const betDoc of betsSnap.docs){
        ...
        await sendTelegramCertificate(ud.telegramChatId, {
          ...
          isWinner: isWin,
          marketTitle: mktData.question,
          side: b.side,
          staked: b.amount || 0,
          payout: winAmt,
          profit,
          newBalance: ud.balance || 0,
          marketSlug: mktData.slug
        });
      }
...
app.post('/api/telegram-webhook', async (req, res) => {
  ...
  try {
    ...
    if (text === '/help') {
      await sendTelegramMessage(chatId,
        ...
        `/refer — Your referral dashboard & link\n` +
        `/setalert — Set market odds alert from bot\n` +
        ...
        { reply_markup: { inline_keyboard: [
          ...
          [{ text: '👥 Refer & Earn', callback_data: 'refer' }, { text: '🔔 Market Alert', callback_data: 'setalert' }],
  ...
  // ── Support AI flow — fires if in support state OR if no other state matches ──
    ...
    if (msg.text && !msg.text.startsWith('/') && supportSt) {
