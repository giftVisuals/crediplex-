      ...
      try {
        ...
        try {
          for(const b of betsSnap.docs){
            const bet = b.data();
            const userSnap = await db.collection('users').doc(bet.uid).get();
            ...
            if(isWinner){
              const amt = bet.isBonus ? Math.floor(((bet.winAmount||0))* 1) : (bet.winAmount||0);
...
_existingPmIdsCache[pmId] = true; // remember it so we don't re-import it before the next cache rebuild
    ...
    // Immediately refresh cache so new markets are live
...
setTimeout(migrateWorldCupCategoryOnce, 20 * 1000);

// Run sync on startup (after 10s) then every 2 minutes
async function runSyncThenRefreshCache() {
  await autoSyncPolymarketMarkets();
  await refreshMarketsCache(); // bust cache so new markets are immediately live
}
setTimeout(runSyncThenRefreshCache, 60 * 1000);
setInterval(runSyncThenRefreshCache, 60 * 60 * 1000);
console.log('🔄 Auto Polymarket sync started (every 60 min, cache auto-refreshed after)');
  ...
  try {
    ...
    for (const mDoc of snap.docs) {
      ...
      try {
        ...
        if (m.resolveType === 'subscribers' && m.targetSubscribers) {
          const target = Number(m.targetSubscribers);
          const deadline = m.deadline?.toDate?.() || new Date(0);
          const isExpired = new Date() >= deadline;

          if (currentCount >= target) {
            // Target hit — resolve YES
...
async function autoResolveTelegramMarket(marketId, m, winner) {
  try {
    ...
    await db.runTransaction(async tx => {
      ...
      // Use fresh pool data from Firestore, not stale in-memory m
      ...
      for (const betDoc of betsSnap.docs) {
        ...
        if (isWinner) {
          const rawWinAmt = winningSidePool > 0
            ? Math.floor((totalPool * 0.90 / winningSidePool) * Number(b.amount || 0))
            : Number(b.amount || 0);
          const betProfit = rawWinAmt - Number(b.amount || 0);
          const betProfitPct = Number(b.amount || 0) > 0 ? (betProfit / Number(b.amount || 0)) * 100 : 0;
          const platformFee = betProfitPct > 10 ? Math.floor(betProfit * 0.10) : 0;
          const winAmt = rawWinAmt - platformFee;
          const netProfit = winAmt - Number(b.amount || 0);

          if (platformFee > 0) {
            const adminRef = db.collection('users').doc('WEw1TEQXJhZhmls7ppb4D0zxMv62');
            tx.update(adminRef, { balance: admin.firestore.FieldValue.increment(platformFee) });
            const feeRef = db.collection('transactions').doc();
            tx.set(feeRef, {
              uid: 'WEw1TEQXJhZhmls7ppb4D0zxMv62', type: 'platform_fee',
              amount: platformFee, fromUser: b.uid, marketId,
              note: 'Platform fee from Telegram market win',
              createdAt: admin.firestore.FieldValue.serverTimestamp()
            });
          }
...
setTimeout(pollTelegramChannelMarkets, 10 * 1000); // first poll after 10s
...
setInterval(async () => {
  try {
    ...
    const snap = await db.collection('markets')
      .where('category','==','Telegram')
      .where('status','==','active')
      .where('deadline','<=', sixHoursFromNow)
      .get();
    if(!snap.empty){
      console.log(`⏰ Near-deadline Telegram poll: ${snap.docs.length} markets`);
  ...
  try {
    ...
    if (text === '/help') {
      await sendTelegramMessage(chatId,
        ...
        `/balance — Check your Crediplex balance\n` +
    ...
    if (text === '/balance' || text === '/portfolio') {
      ...
      if (text === '/balance') {
        await sendTelegramMessage(chatId,
          ...
          `<a href="https://crediplex.ng">Open Crediplex →</a>`
        );
      } else {
        ...
        await sendTelegramMessage(chatId,
          ...
          `<a href="https://crediplex.ng">View full portfolio →</a>`
        );
      }
      return;
    }
...
app.post('/api/create-telegram-market', async (req, res) => {
  try {
    const { question, telegramChannel, telegramLink, targetSubscribers, deadline, description, imageUrl, currentSubscribers } = req.body;
...
const MARKETS_CACHE_TTL = 10 * 60 * 1000; // 10 minutes
...
async function refreshMarketsCache() {
  try {
    ...
    // Paginate in batches of 500 until Firestore returns empty
    while (true) {
...
setTimeout(refreshMarketsCache, 5000);
...
app.get('/api/markets', async (req, res) => {
  // If cache is empty or stale, build it now
  if (!_marketsCache.length || Date.now() - _marketsCacheBuiltAt > MARKETS_CACHE_TTL) {
    await refreshMarketsCache();
  }
...
const CRYPTO_MARKETS_CACHE_TTL = 10 * 60 * 1000; // 10 minutes
...
setTimeout(refreshCryptoMarketsCache, 7000);
...
setTimeout(refreshTopTradersCache, 9000);
...
app.get('/api/top-traders', async (req, res) => {
  ...
  res.json({ success: true, traders: _topTradersCache });
});
...
const USER_BETS_CACHE_TTL = 2 * 60 * 1000;
...
async function checkPriceAlerts(){
  try {
    ...
    const usersSnap = await db.collection('users').where('priceAlerts','!=',null).select('priceAlerts','telegramChatId','username').get();
        ...
        if(currentOdds >= Number(alert.threshold)){
          if(u.telegramChatId){
            ...
            await sendTelegram(u.telegramChatId,
              ...
              `<a href="https://crediplex.ng">Bet now on Crediplex →</a>`
            );
          }
