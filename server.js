...
for(const b of betsSnap.docs){
  ...
  if(isWinner){
    ...
    if(fcmToken) await sendFcmToToken(fcmToken, '🎉 You Won!', `You won ₦${amt.toLocaleString()} on "${shortQ}"`);
    if(tgId) await sendTelegramMessage(tgId, `🎉 <b>You Won!</b>\n\nYou won <b>₦${amt.toLocaleString()}</b> on:\n"${shortQ}"\n\n💰 Check your wallet on Crediplex!`);
  } else {
    if(fcmToken) await sendFcmToToken(fcmToken, '😔 Bet Lost', `Your bet on "${shortQ}" did not win this time.`);
    if(tgId) await sendTelegramMessage(tgId, `😔 <b>Bet Lost</b>\n\nYour bet on:\n"${shortQ}"\n\ndid not win this time. Better luck next time! 💪`);
  }
}
