...
app.post('/api/telegram-webhook', async (req, res) => {
  ...
  try {
    ...
    if (text === '/markets' || text.startsWith('/markets ')) {
      ...
      let pool = _marketsCache.filter(m => m.status === 'active');
      ...
      if (!pool.length) {
        await sendTelegramMessage(chatId,
          ...
          { reply_markup: { inline_keyboard: [[{ text: '📈 View on App', url: 'https://crediplex.name.ng' }]] }}
        ); return;
      }
      ...
      const buttons = [[{ text: '🔍 View All Markets', url: 'https://crediplex.name.ng' }]];
    ...
    if (text === '/mybets') {
      ...
      const lines = allBets.slice(0,10).map((b, i) => {
        const q = (marketMap[b.marketId] || 'Unknown market').substring(0, 55);
    ...
    if (text === '/lb_polymarket') {
      // Fetch from Railway cache (0 reads)
      try {
        const r = await fetch(`${process.env.SERVER_URL || 'https://crediplex-production.up.railway.app'}/api/top-traders`, { signal: AbortSignal.timeout(5000) });
        const data = await r.json();
        const traders = (data.traders || []).slice(0, 5);
        if (!traders.length) { await sendTelegramMessage(chatId, `⚠️ Polymarket data loading, try again shortly.`); return; }
        const medals = ['🥇','🥈','🥉','4️⃣','5️⃣'];
        const lines = traders.map((u, i) => {
          const vol = u.volume ? `$${Number(u.volume).toLocaleString()}` : 'N/A';
          const wr = u.winRate || u.pnl || 0;
          return `${medals[i]} <b>${u.name || u.username || 'Trader'}</b>\n   Vol: ${vol} · PnL: ${wr}%`;
        }).join('\n\n');
        await sendTelegramMessage(chatId,
          `📊 <b>Top Polymarket Traders</b>\n\n${lines}\n\n<i>Copy any of these traders on Crediplex!</i>`,
          { reply_markup: { inline_keyboard: [
            [{ text: '🌐 Crediplex Leaderboard', callback_data: 'lb_crediplex' }],
            [{ text: '🔄 Copy Traders', url: 'https://crediplex.name.ng' }]
          ]}}
        );
      } catch(e) { await sendTelegramMessage(chatId, `⚠️ Could not load Polymarket data. Try again shortly.`); }
      return;
    }
