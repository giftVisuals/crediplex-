  ...
  try {
    ...
    for (const mDoc of snap.docs) {
      ...
      try {
        ...
        if (updateData._autoResolve) {
          await autoResolveTelegramMarket(mDoc.id, m, updateData._autoResolve);
        }
...
async function autoResolveTelegramMarket(marketId, m, winner) {
  try {
      ...
      for (const betDoc of betsSnap.docs) {
        ...
        if (isWinner) {
          const rawWinAmt = winningSidePool > 0
            ? Math.floor((totalPool * 0.90 / winningSidePool) * Number(b.amount || 0))
            : Number(b.amount || 0);
          const profit = rawWinAmt - Number(b.amount || 0);

          tx.update(betRef, { status: 'won', winAmount: rawWinAmt });
          tx.update(db.collection('users').doc(b.uid), {
            balance: admin.firestore.FieldValue.increment(rawWinAmt),
            wins: admin.firestore.FieldValue.increment(1),
            profit: admin.firestore.FieldValue.increment(profit)
          });
          const txRef = db.collection('transactions').doc();
          tx.set(txRef, {
            uid: b.uid, type: 'win_payout', amount: rawWinAmt,
            note: `Won: ${(m.question || '').substring(0, 60)}`,
            createdAt: admin.firestore.FieldValue.serverTimestamp()
          });
        } else {
    ...
    console.log(`✅ Auto-resolved Telegram market "${(m.question||'').substring(0,40)}" → ${winner}`);
  } catch (e) {
...
setTimeout(pollTelegramChannelMarkets, 30 * 1000);
setInterval(pollTelegramChannelMarkets, 30 * 60 * 1000);
console.log('📱 Telegram channel poller started (every 30 min)');
