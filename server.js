...
for(const b of betsSnap.docs){
  ...
  if(isWinner){
    ...
    if(tgId) await sendTelegramMessage(tgId,
      `🎉 <b>You Won!</b>\n\n` +
      `Market: <i>${shortQ}</i>\n\n` +
      `Your side: <b>${bet.side}</b> ✅\n` +
      `Staked: <b>₦${Number(staked).toLocaleString('en-NG')}</b>\n` +
      `Payout: <b>₦${Number(amt).toLocaleString('en-NG')}</b>\n` +
      `Profit: <b>+₦${Number(profit).toLocaleString('en-NG')}</b> 🚀\n\n` +
      `New Balance: <b>₦${Number(newBal).toLocaleString('en-NG')}</b>`,
      { reply_markup: { inline_keyboard: [
        [{ text: '📈 Bet Again', url: 'https://crediplex.name.ng' }, { text: '💰 My Balance', callback_data: 'balance' }]
      ]}}
    );
  } else {
    const staked = bet.amount || 0;
    ...
    if(tgId) await sendTelegramMessage(tgId,
      ...
      `Market: <i>${shortQ}</i>\n\n` +
      `Your side: <b>${bet.side}</b> ❌\n` +
      `Amount lost: <b>₦${Number(staked).toLocaleString('en-NG')}</b>\n\n` +
      `Balance: <b>₦${Number(newBal).toLocaleString('en-NG')}</b>\n\n` +
      `Don't give up — the next one is yours! 💪`,
      { reply_markup: { inline_keyboard: [
        [{ text: '📈 Try Again', url: 'https://crediplex.name.ng' }, { text: '📊 My Stats', callback_data: 'portfolio' }]
      ]}}
    );
  }
}
...
_existingPmIdsCache[pmId] = true; // remember it so we don't re-import it before the next cache rebuild
    ...
    // ── THIRD PASS: fetch ALL markets directly from clob API (catches ones missed by events endpoint) ──
    try {
      let clobPage = 0;
      const clobSize = 500;
      while (clobPage <= 60) { // up to 30,000 markets
    ...
    const tagFetches = [
      'sports', 'crypto', 'politics', 'elections', 'science',
      'entertainment', 'weather', 'business', 'tech',
      'world-cup', 'soccer', 'nigeria', 'africa'
    ];
    ...
    for (const tag of tagFetches) {
      try {
        ...
        for (const pm of tagMarkets) {
          ...
          // World Cup tag check — force category
          const tagStr = pmTags.join(' ').toLowerCase();
          let category;
          if (tagStr.includes('world-cup') || tagStr.includes('soccer') || tag === 'world-cup' || tag === 'soccer') {
            const q = (pm.question || '').toLowerCase();
            if (q.includes('world cup') || q.includes('fifa') || tagStr.includes('world-cup')) {
              category = 'World Cup';
            } else {
...
app.post('/api/telegram-webhook', async (req, res) => {
  ...
  try {
    ...
    // Rate limit check
    if (isTgRateLimited(chatId.toString())) return;
    ...
    if (text === '/news') {
      ...
      const sorted = _marketsCache
        .filter(m => m.status === 'active' && m.polymarketVolume > 0)
        .sort((a, b) => (b.polymarketVolume || 0) - (a.polymarketVolume || 0))
        .slice(0, 3);
      if (!sorted.length) {
        await sendTelegramMessage(chatId, `📰 No trending markets right now. Check back soon!`); return;
      }
      ...
      try {
        ...
        await sendTelegramMessage(chatId,
          ...
          { reply_markup: { inline_keyboard: [
            [{ text: '📈 View Markets', url: 'https://crediplex.name.ng' }, { text: '🔔 Set Alert', callback_data: 'alert' }]
          ]}}
        );
      } catch(aiErr) {
    ...
    if (text.startsWith('/start')) {
      ...
      if (!existingUser.empty) {
        ...
        await sendTelegramMessage(chatId,
          ...
          `Use /balance to check your wallet or /portfolio to see your stats.\n\n` +
          `To stop receiving notifications, type /unsubscribe.`
        );
        return;
      }
    ...
    if (text === '/unsubscribe') {
      ...
      if (userSnap.empty) {
        await sendTelegramMessage(chatId, `⚠️ No Crediplex account is linked to this Telegram.`);
        return;
      }
      await userSnap.docs[0].ref.update({ telegramChatId: admin.firestore.FieldValue.delete() });
      await sendTelegramMessage(chatId,
        `✅ <b>Unsubscribed successfully.</b>\n\n` +
        `You will no longer receive Crediplex notifications here.\n\n` +
        `To re-link your account, type /start anytime.`
      );
      return;
    }
    ...
    if (text === '/help') {
      await sendTelegramMessage(chatId,
        ...
        `/deposit — Fund your Crediplex wallet\n` +
        `/markets — Top 3 live prediction markets\n` +
        `/news — AI-powered trending market news\n` +
        `/mybets — Your active bets\n` +
        `/alert — Set a price alert on any market\n` +
        `/leaderboard — Top 5 traders by profit\n` +
        `/support — Chat with AI support agent\n` +
        `/cancel — Cancel any in-progress action\n` +
        `/start — Link your Crediplex account\n` +
        `/unsubscribe — Stop notifications & unlink\n` +
        `/help — Show this menu`,
        { reply_markup: { inline_keyboard: [
          [{ text: '💰 Balance', callback_data: 'balance' }, { text: '📊 Portfolio', callback_data: 'portfolio' }],
          [{ text: '💳 Deposit', callback_data: 'deposit' }, { text: '👥 Refer', callback_data: 'refer' }],
          [{ text: '📰 News', callback_data: 'news' }, { text: '🎯 My Bets', callback_data: 'mybets' }],
          [{ text: '🔔 Set Alert', callback_data: 'alert' }, { text: '🏆 Leaderboard', callback_data: 'leaderboard' }],
          [{ text: '🎧 Support', callback_data: 'support' }, { text: '🚀 Open App', url: 'https://crediplex.name.ng' }]
        ]}}
      );
      return;
    }
    ...
    if (text === '/refer') {
      ...
      await sendTelegramMessage(chatId,
        `👥 <b>${u.username}'s Referral Info</b>\n\n` +
        `Your Code: <code>${refCode}</code>\n` +
        `Tier: <b>🏅 ${tier}</b>\n` +
        `Referrals: <b>${refCount}</b>\n` +
        `Volume Generated: <b>₦${Number(refVolume).toLocaleString('en-NG')}</b>\n\n` +
        `Share your link:\n<code>https://crediplex.name.ng?ref=${refCode}</code>`,
        { reply_markup: { inline_keyboard: [[{ text: '🚀 Open Crediplex', url: `https://crediplex.name.ng?ref=${refCode}` }]] } }
      );
      return;
    }
    ...
    if (text === '/deposit_crypto') {
      ...
      await sendTelegramMessage(chatId,
        `💵 <b>Crypto Deposit</b>\n\n` +
        `Deposit with USDT, BTC, SOL, ETH and more.\n\n` +
        `Minimum: <b>$1 USD</b>\n` +
        `Auto-credited within ~2 minutes after network confirmation.\n\n` +
        `Tap the button below to open the deposit page in Crediplex:`,
        { reply_markup: { inline_keyboard: [[{ text: '🔗 Open Crypto Deposit', url: 'https://crediplex.name.ng/#deposit' }]] } }
      );
      return;
    }
    ...
    if (text === '/deposit_naira') {
      ...
      await sendTelegramMessage(chatId,
        ...
        `Account Name: <b>${NAIRA_ACCOUNT_NAME}</b>\n` +
        `Account Number: <code>${NAIRA_ACCOUNT_NUMBER}</code>\n\n` +
    ...
    if (text === '/markets') {
      // 0 reads — uses _marketsCache
      const top = _marketsCache.filter(m => m.status === 'active').slice(0, 3);
      if (!top.length) {
        await sendTelegramMessage(chatId, `⚠️ No active markets right now. Check back soon!`); return;
      }
      const lines = top.map((m, i) => {
        const odds = m.polymarketYesOdds || 50;
        return `${i+1}. <b>${(m.question||'').substring(0,70)}</b>\n   YES: ${odds}% | NO: ${100-odds}%`;
      }).join('\n\n');
      await sendTelegramMessage(chatId,
        `📈 <b>Top Live Markets</b>\n\n${lines}`,
    ...
    if (text === '/mybets') {
      ...
      const betsSnap = await db.collection('bets').where('uid', '==', uid).where('status', '==', 'pending').orderBy('createdAt', 'desc').limit(10).get();
      if (betsSnap.empty) {
      ...
      const lines = betsSnap.docs.map((d, i) => {
        const b = d.data();
        const q = (marketMap[b.marketId] || 'Unknown market').substring(0, 60);
        return `${i+1}. <b>${b.side}</b> — ₦${Number(b.amount||0).toLocaleString('en-NG')}\n   📌 ${q}`;
      }).join('\n\n');
      await sendTelegramMessage(chatId,
        `🎯 <b>Your Active Bets (${betsSnap.size})</b>\n\n${lines}`,
        { reply_markup: { inline_keyboard: [[{ text: '📈 View All on Crediplex', url: 'https://crediplex.name.ng/#portfolio' }]] }}
      );
      return;
    }
    ...
    if (text === '/leaderboard') {
      // 0 reads — uses _topTradersCache
      const top5 = _topTradersCache.slice(0, 5);
      if (!top5.length) {
        await sendTelegramMessage(chatId, `⚠️ Leaderboard loading, try again in a minute.`); return;
      }
      const medals = ['🥇','🥈','🥉','4️⃣','5️⃣'];
      const lines = top5.map((u, i) => {
        const profit = Number(u.profit || 0);
        const wr = u.winRate || 0;
        return `${medals[i]} <b>${u.username || 'User'}</b> — ₦${profit.toLocaleString('en-NG')} profit (${wr}% WR)`;
      }).join('\n');
      await sendTelegramMessage(chatId,
        `🏆 <b>Top 5 Traders</b>\n\n${lines}\n\n<a href="https://crediplex.name.ng">View full leaderboard →</a>`,
        { reply_markup: { inline_keyboard: [[{ text: '🏆 Full Leaderboard', url: 'https://crediplex.name.ng/#leaderboard' }]] } }
      );
      return;
    }
  ...
  // ── Unknown command fallback ──
    if (text.startsWith('/') && !_alertState[chatId.toString()]) {
      await sendTelegramMessage(chatId,
        `🤔 I don't know that command.\n\nType /help to see everything I can do.`,
        { reply_markup: { inline_keyboard: [
          [{ text: '📋 Show Commands', callback_data: 'help' }]
        ]}}
      );
      return;
    }
  ...
  // ── Support AI flow ──
    ...
    if (supportSt && msg.text && !msg.text.startsWith('/')) {
      const userMessage = msg.text.trim();
      supportSt.messages.push({ role: 'user', content: userMessage });
      // Keep last 6 messages for context (0 reads)
      if (supportSt.messages.length > 6) supportSt.messages = supportSt.messages.slice(-6);
      try {
        const reply = await askGroq([
          { role: 'system', content: CREDIPLEX_SUPPORT_PROMPT },
          ...supportSt.messages
        ], 300);
        supportSt.messages.push({ role: 'assistant', content: reply });
        const needsEscalation = reply.toLowerCase().includes('care@crediplex') || reply.toLowerCase().includes('complex') || reply.toLowerCase().includes('team');
        ...
        await sendTelegramMessage(chatId,
          `Sorry, I'm having trouble right now. Please email care@crediplex.name.ng and our team will help you within 24 hours. 🙏`,
          { reply_markup: { inline_keyboard: [[{ text: '📧 Email Support', url: 'mailto:care@crediplex.name.ng' }]] }}
        );
      }
      return;
    }
  ...
  // ── Alert flow: handle freetext keyword and threshold replies ──
    ...
    if (alertSt) {
      ...
      if (alertSt.step === 'admin_broadcast_awaiting_message') {
        ...
        const caption = update.message?.caption || msg.text || '';
        try {
            ...
            try {
              if (photo && photo.length > 0) {
                ...
                await fetch(`${TELEGRAM_API}/sendPhoto`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ chat_id: tgId, photo: fileId, caption: `📢 <b>Crediplex Update</b>\n\n${caption}`, parse_mode: 'HTML' })
                });
              } else {
          ...
          await sendTelegramMessage(chatId, `✅ Broadcast sent to <b>${sent}</b> users.`);
...
let _lastTgPollAt = 0;
const MARKETS_CACHE_TTL = 2 * 60 * 60 * 1000; // 2 hours — halves cache rebuild reads — refreshed by sync or on stale check
...
async function refreshMarketsCache() {
  ...
  try {
    ...
    // Paginate in batches of 500, cap at 10,000 markets to prevent OOM
    const MAX_MARKETS = 10000;
    while (allDocs.length < MAX_MARKETS) {
      let q = db.collection('markets')
        .where('status', '==', 'active')
    ...
    if (allDocs.length >= MAX_MARKETS) {
      console.warn(`⚠️ Markets cache hit ${MAX_MARKETS} limit — older markets may not be in cache`);
    }
...
let _cryptoMarketsCache = [];
let _cryptoMarketsCacheBuiltAt = 0;
const CRYPTO_MARKETS_CACHE_TTL = 60 * 60 * 1000; // 1 hour (crypto markets rarely change)

async function refreshCryptoMarketsCache() {
  try {
    const snap = await db.collection('cryptoMarkets').where('status', '==', 'active').get();
    _cryptoMarketsCache = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    _cryptoMarketsCacheBuiltAt = Date.now();
    console.log(`✅ Crypto markets cache refreshed: ${_cryptoMarketsCache.length} markets`);
  } catch (e) {
    console.error('refreshCryptoMarketsCache error:', e.message);
  }
}
setTimeout(refreshCryptoMarketsCache, 25000);
setInterval(refreshCryptoMarketsCache, CRYPTO_MARKETS_CACHE_TTL);

app.get('/api/crypto-markets', async (req, res) => {
  if (!_cryptoMarketsCache.length || Date.now() - _cryptoMarketsCacheBuiltAt > CRYPTO_MARKETS_CACHE_TTL) {
    await refreshCryptoMarketsCache();
  }
  res.setHeader('Cache-Control', 'public, max-age=60');
  res.json({ success: true, markets: _cryptoMarketsCache, count: _cryptoMarketsCache.length });
});
...
setTimeout(refreshTopTradersCache, 40000);
setInterval(refreshTopTradersCache, TOP_TRADERS_CACHE_TTL);
...
const NOTIF_CACHE_TTL = 15 * 60 * 1000;
...
app.get('/api/recent-bets', async (req, res) => {
  if(_recentBetsCache && Date.now() - _recentBetsCacheAt < 60000){
        ...
        if(currentOdds >= Number(alert.threshold)){
          if(u.telegramChatId){
            await sendTelegramMessage(u.telegramChatId,
              ...
              { reply_markup: { inline_keyboard: [[{ text: '📈 Bet Now', url: 'https://crediplex.name.ng' }, { text: '🔔 New Alert', callback_data: 'alert' }]] }}
            );
          }
